...
- `/admin/room/{id}` (the room detail page) can keep rendering pre-edit data for 30s+ after a successful `PUT /api/room/{id}` — confirmed by polling it directly for 30s straight after an update. Not an HTTP/CDN cache (the response sends `Cache-Control: no-store` and `cf-cache-status: DYNAMIC`); `GET /api/room/{id}` itself reflects the change in well under a second, so this looks like a server-side rendering/data-cache bug specific to that page. Verify edits via the API rather than this page.
